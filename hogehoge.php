...
try {
    $pdo = new
PDO("mysql:host=localhost;dbname=好きなデータベース名;charset=utf8","root",array(PDO::ATTR_EMULATE_PREPARES
=> false));
$item_name = $_POST["textbox"];
if ($item_name && isset($_POST["btn"])) {
unset($_POST["btn"]);
$item_point = $_POST["number"];
$stmt = $pdo -> prepare("INSERT INTO 好きなテーブル名(name, point) VALUES (:name, :point)");
$stmt->bindParam(':name', $item_name, PDO::PARAM_STR);
$stmt->bindValue(':point', $item_point, PDO::PARAM_INT);
$stmt->execute();
}
$select_sql = "SELECT * FROM 好きなテーブル名";
